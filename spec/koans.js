...
describe("the JavaScript language", function() {
  ...
  describe("has different types and operators", function() {
    ...
    it("considers an empty string to be falsy", function() {
      expect("" == false).toBeTruthy();// Truthy or Falsy //tipo de variable es igual tanto string como boolean
      expect("" === false).toBeFalsy();// Truthy or Falsy
    });

    it("considers zero to be falsy", function() {
      expect(0 == false).toBeTruthy();// Truthy or Falsy //tipo de variable se considera igual tanto string como boolean
    ...
    it("considers nulls to be falsy", function() {
      ...
      expect(null === false).toBeFalsy();// Truthy or Falsy //Estas dos siguientes no tienen relación con el if
      expect(null == false).toBeFalsy();// Truthy or Falsy //null no cuenta como valor verdadero de ningun tipo porque no existe
    });
    ...
    it("knows the type of a function", function() {
      ...
      expect(typeof(xxx)).toBe('undefined'); //es un valor indefinido
    });
    ...
    it("may contain functions inside arrays", function() {
      ...
      expect(arr[2](1)).toEqual(4); //le asignamos el número 1 como arg a la función a través de su index
    });
    ...
    it("concatenate arrays - well, kind of", function() {
      ...
      expect(a + b).toEqual("1,2,34,5,6"); //cuando concatenas arrays se convierten en string
    });
    ...
    it("joins arrays and other things", function() {
      ...
      expect(true + a).toEqual("true1,2,3"); //los booleanos los convierte en string
    });
    ...
    it("is not the same to compare by value than by reference ", function() {
      ...
      expect(a).toEqual(b);        // Jasmine toEqual compares by value //valor numérico, o string son iguales en este caso porque son array
      expect(a).not.toBe(b);       // Jasmine toBe compares by reference //no son iguales porque no son la misma variable. Referencia es que te refieras a la misma variable
    });
  });
  ...
  describe("considers functions as first class citizens", function() {
    ...
    it("can declare anonymous functions", function() {
      ...
      expect(someVar(1,1)).toBe(2); //Esta sí suma
    });
    ...
    it("may return arrays that contains functions and so on", function() {
      ...
      expect(example()[0](1)[1]).toEqual(10); //ejecuta la función, entra al array y a su index 0, la cual tiene una función que recibe el valor 1, modifica un array del indice 0 a 1 dentro de la función. En este último paso manda el resultado de 10.
    });
    ...
    it("matters, the declaration order when they are anonymous", function() {
      ...
      expect(exampleA()).toEqual(1); //lo movimos de lugar :D
    });

    it("can use optional parameters", function() {
      function example(a, b, c) {
        if (c) { //si existe c entonces ejecuta
          return a + b + c; 
        }
        return a + b;
      }
    ...
    it("anonymous functions are anonymous", function() {
      ...
      expect(typeof(z)).toEqual('undefined'); //No se puede llamar a la función porque sólo existe dentro de la variable
      expect(x()).toEqual(1);
    });
    ...
    it("can create closures with several free variables", function() {
      function external() {
        var a = 1, b = 2; //Estamos declarando dos variables al mismo tiempo
    ...
    it("passes arrays by reference", function() {
      ...
      example(x);
      expect(x).toEqual([100,2,3]); //no imprime el arreglo, sólo lo modifica, por eso va entre corchetes
    });
    ...
    it("passes objects by reference", function() {
      ...
      example(x);
      expect(x).toEqual({ property: "test" }); //sustituye la propieda por el nuevo string (igual que el de arriba)
    });
    ...
    it("may return a function as the result of invoking a function", function() {
      ...
      function example(){
        return add; //hace referencia a la función sólo con su nombre
      }

      expect(example()(1,2)).toEqual(3);//ejecutas la primera función, como su resultado es una referencia a otra función, le asignas los parámetros de la siguiente función
      var f = example();
      expect(f(2,2)).toEqual(4);//se pueden pasar los argumentos de una función que hace referencia a otra función
    });
    ...
    it("can invoke functions indirectly using the special 'apply'", function() {
      ...
      expect(f.apply(f, [1,1])).toEqual(2); //a diferencia del anterior este utiliza []
    });
    ...
    it("doesnt have a private scope inside blocks", function() {
      ...
      expect(i).toEqual(5);//es el ultimo valor que recibe i
      expect(j).toEqual(10); //la iteración sólo ocurre hasta el número 4 (como si fuera un factorial)
    });
  });
  ...
  describe("has multiple ways to define and create objects", function() {
    ...
    it("can create properties dynamically", function() {
      ...
      expect(obj["address"]).toEqual("palm tree"); //otra manera de llamar keys
    ...
    it("may define complex objects", function() {
      ...
      expect(user.friends[0].name).toEqual('triki'); //utiliza corchetes y llaves
    });
    ...
    it("has a pattern called, the Module Pattern", function() {
      function createObject() {
        ...
        return {
          addPoint: function(){ ++points; }, //incrementa 0 a 1 ñ.ñ
      ...
      expect(typeof(obj.points)).toEqual('undefined'); //es undefined porque points es una variable
    });
  ...
  describe("commons patterns with functions and behaviors", function() {
    ...
    it("hoists variables the way you probably dont expect", function() {
      ...
      expect(generate()[0]()).toEqual(5); //Por qué es un 5? -.-''
      expect(generate()[1]()).toEqual(5); //Por qué es un 5? -O-''!!
    });
  });
  ...
  context("has ways to simulate classes", function() {
    ...
    function Lion(energy) {
      ...
      var self = this; //Qué es this en este caso?
    ...
    context("and the THIS keyword", function() {
      ...
      it("works different on dettached functions", function() {
        ...
        expect(cat.kilos).toEqual(1); //¿por qué esto es 1? T_T
      });
      ...
      it("can be bound explicitly with CALL and APPLY", function() {
        ...
        expect(cat.kilos).toEqual(2); //No entiendo bien :,C
      });

      it("can be bound in modern browsers with BIND", function() {
        var feed = cat.feed;
        var bound = feed.bind(cat); //cuando creas una variable y le asignas una función, puedes llamara directamente por su nombre y ()
