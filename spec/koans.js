...
describe('the JavaScript language', () => {
  describe('has different types and operators', () => {
    ...
    it('considers an empty string to be falsy', () => { //mirar luego
    ...
    it("can't compare arrays", () => { // por qué???
      const a = [1, 2, 3];
      const b = [1, 2, 3];
  ...
  describe('considers functions as first class citizens', () => {
    ...
    it('may return arrays that contains functions and so on', () => {
      function example() {
        return [function (n) { return [n + 2, null, 2] }, 4, -1]
      }

      expect(example()[0](1)[1]).toEqual(null);
    });
    ...
    it('matters, the declaration order when they are anonymous', () => {
      ...
      expect(exampleA()).toEqual(ReferenceError); // por qué Jasmine dice no?
    ...
    it('can create closures with several free variables', () => {
      ...
      expect(external()).toBe(undefined); // por qué???
    });
