...
//ASK
    ...
    //Adding arrays turns them into strings 
      const a = [1, 2, 3];
      const b = [4, 5, 6];
...
//STUDY: "return" (newline) bracket returns error
      function example() {
        return [
          function(num){
            return [, num+9]
...
//ASK: why does function f(2,2) =4, but example(1,2) does not work?
      function add(a, b) {
        return a + b;
      }
...
//i is undefined, but Jasmine still returns error "i is not defined"
//The only way we could get it to work was by commenting out the line
      let j = 0;
      for (let i = 0; i < 5; i++) {
        j += i;
      }
...
//ASK: why expect(typeof(obj.points)).toEqual('undefined'); should be number?
    ...
    it('may create objects also with the module pattern', () => {
      function createObject(initialScore, someColor) {
