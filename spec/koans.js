...
describe('the JavaScript language', () => {
  describe('has different types and operators', () => {
    ...
    it('considers nulls to be falsy', () => {
      ...
      expect(result == false).toBe( result == false);// Truthy or Falsy
      expect(null === false).toBe(null === false);// Truthy or Falsy
      expect(null == false).toBe(null === false);// Truthy or Falsy
    });
    ...
    it("can't compare arrays", () => {
      ...
      expect(a == b).toBeFalsy();  // Truthy or Falsy ***** DUDA DE PORQUE ES LO MISMO CON (...).toBe(a == b);
      expect(a === b).toBeFalsy(); // Truthy or Falsy ***** DUDA DE PORQUE ES LO MISMO CON (...).toBe(a == b);
    });
  ...
  describe('considers functions as first class citizens', () => {
    ...
    it('can use optional parameters', () => {
      ...
      expect(example(1,1,1)).toBeEqual(3);
      expect(example(1,1)).toBeEqual(2);
    });
    ...
    it('can create closures with several free variables', () => { //a qué se refiere con closures
      function external() {
        const a = 1,
          b = 2;
        function internal() {
      ...
      expect(external()).toBe(external(6));
    });
