...
describe('the JavaScript language', () => {
  describe('has different types and operators', () => {
    ...
    it('considers nulls to be falsy', () => {
      ...
      expect(result == false).toBeTruthy();//PQ??????
    ...
    it('concatenate arrays - well, kind of', () => {
      ...
      expect(a + b).toEqual('1,2,34,5,6');//CONCATENA SEM ESPAÇO?
    });

    it('joins arrays and strings', () => {
      const a = [1, 2, 3];

      expect ("1" + a).toEqual('11,2,3'); //PQ??? os espaços tbm nao entram! 
    ...
    it('joins arrays and other things', () => {
      ...
      expect(true + a).toEqual('true1,2,3'); //PQ?? TRUE É TRUE? NÃO SERIA UM NUMERO? 
    });
    ...
    it("can't compare arrays", () => {
      ...
      expect(a == b).toBeFalsy();  //DOIS ARRAYS NUNCA VAO SER IGUAIS, SÓ SE EU COMPARAR OS VALORES, ver linha 127
  ...
  describe('considers functions as first class citizens', () => {
    ...
    it('matters, the declaration order when they are anonymous', () => {
      const exampleB = function(arg1) { //TROQUEI A ORDEM 
        return arg1;
      };
    ...
    it('may return arrays that contains closures and so on', () => { //NÃO SEIII 
      function example() {
    ...
    it('passes arrays by reference', () => {
      ...
      example(x);
      expect(x).toEqual([100, 2, 3]); //PQ O INDEX 1 NAO APARECEU??
    });
    ...
    it('can return closures as a function result', () => {
      ...
      expect(f(3)).toBe(8); //AMOUNT = 5 E NUMBER = 3??
    });
    ...
    it('may have functions as the input and the output', () => {
      ...
      expect(f(2)).toBe(3); //AQUI A MINHA FUNCTION É IGUAL A DOIS?? 
    });
  ...
  describe('has multiple ways to define and create objects', () => {
    ...
    it('has a pattern called, the Module Pattern', () => {
      ...
      expect(obj.score()).toEqual(1); //NAO ENTENDI!
    ...
    it('can have have methods in the prototype', () => {
      ...
      Obj.prototype.theName = function() { //O QUE O PROTOTYPE?
        return this.name;
      };
  ...
  context('has ways to simulate classes', () => {
    ...
    context('and the THIS keyword', () => {
      ...
      it('interprets the THIS when the function is executed', () => {
        ...
        lion.hunt = function() {
          this.energy = 4000; //REVER ESSE CONCEITO 
        };
        lion.hunt();
