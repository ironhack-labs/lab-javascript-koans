...
describe('the JavaScript language', () => {
  describe('has different types and operators', () => {
    ...
    it('surprises me, NaN is not comparable with NaN', () => { // do i have ti modify this? 
      expect(5 / 'a').toEqual(5 / 'a');
      expect(typeof(NaN)).toEqual("NaN"); // huh???????
    ...
    it('considers zero to be falsy', () => {
      expect(0 == false).toBeTruthy();//  dont know why :D
    ...
    it('knows the type of a function', () => {
      ...
      //expect(typeof(xxx)).toBe('...'); // why is this part here??
    });
    ...
    it('is not the same to compare by value than by reference ', () => { // am i supposed to edit this part?
      const a = [1, 2, 3];
      const b = [1, 2, 3];
  ...
  describe('considers functions as first class citizens', () => {
    it('can declare named functions', () => {
      ...
      expect(example("whatever")).toEqual("some example");
    });
    ...
    it('may return arrays that contains functions and so on', () => {
      function example() {
        ...
        const arr = [(number) => [123, 10] ];
        return arr;
      }

      expect(example()[0](1)[1]).toEqual(10);
    });
// k i dont feel like im really learning anything here, more just hurting my brain while trying to print 10 to a console i cant see
