...
// NaN is a falsy number. typeof number does not garantee actual numbers =O
...
// empty string is a falsy value. So, with two equals, there is type conversion from string to boolean: false === false logs truthy
...
// same reasoning as above
    ...
    it('considers nulls to be falsy', () => {
      ...
      // null is not saved on memory (as undefined) and can't be type converted.
      expect(null == false).toBeFalsy();// Truthy or Falsy
    });
...
// because x doesn't return anything?
    ...
    it('concatenate arrays - well, kind of', () => {
      const a = [1, 2, 3];
      const b = [4, 5, 6];
// why?? idk. It treats as string.
      expect(a + b).toEqual('1,2,34,5,6');
    });

    it('joins arrays and strings', () => {
      const a = [1, 2, 3];
// treat it as string before concat
    ...
    it("can't compare arrays", () => {
      const a = [1, 2, 3];
      const b = [1, 2, 3];
      // they are different arrays (different refs)
  ...
  describe('considers functions as first class citizens', () => {
    it('can declare named functions', () => {
      ...
      expect(example()).toEqual('some example'); // function call equals its return
    });
    ...
    it('may return arrays that contains functions and so on', () => {
      function example() {
        ...
        return [function(arg) {return [, 10] }]}

      expect(example()[0](1)[1]).toEqual(10);
    });

    // when they are name and created with function declaration, because hoisting
    ...
    // isn't it because it is a function expression?
...
// is that ok?
    ...
    it('may have functions as the input and the output', () => {
      const plus = (originalFunction) => {
        return (arg1) => {
          return originalFunction() + arg1;
        };
      }
      const f = plus(() => 1); // chama a funcao plus, passando 1 como argumento. // retorna a funcao anonima com arg1. p. ex.: originalFunction() + 1
      // salva o retorno em f
      // chama f (closure) passando 2 como argumento
      // f e passado como argumento de plus
      // plus e so uma closure de original func...

      // nomear a original function e testar.
      expect(f(2)).toBe(3); // i can't read it here. 
    });
...
// nao sabia
    ...
    it("can invoke functions indirectly using the special 'apply'", () => {
      ...
      // isso e necessario no contexto do ES6? Quando usar isso?
  ...
  describe('has multiple ways to define and create objects', () => {
    ...
    it('has a pattern called, the Module Pattern', () => {
      ...
      expect(typeof(obj.points)).toEqual('undefined'); // funcao nao retorna nada. apenas modifica a a variavel points que e passada para score.
    });
