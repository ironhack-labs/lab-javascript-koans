...
//AQUI EMPIEZO YO
  ...
  describe('considers functions as first class citizens', () => {
    ...
    it("doesn't care about the declaration order when they are named", () => {
      ...
      expect(exampleA()).toEqual(1); //como tiene nombre si la encuentra sin importar que este declarada despues
    ...
    it('matters, the declaration order when they are anonymous', () => {
      ...
      expect(exampleA()).toEqual(1); //si es anonima vale malles, tiene que estar el expect abajo de la declaracion
...
describe('has multiple ways to define and create objects', () => {
  ...
  it('may create objects also with the module pattern', () => {
      function createObject(initialScore, color) {
          // write the code here
          this.points = () => this.incrementScoreIn(initialScore)
          this.incrementScoreIn = (score) => Number(score + 5)
          this.color = color;
      }
      const obj = new createObject(5, 'red');
      return obj.incrementScoreIn(5);
