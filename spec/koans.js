...
describe('the JavaScript language', () => {
  ...
  describe('considers functions as first class citizens', () => {
    ...
    it('may return arrays that contains functions and so on', () => {
      function example() {
        ...
        return [arg => [arg, arg*10]]; //uau... não acredito que eu consegui
      }
    ...
    it('matters, the declaration order when they are anonymous', () => {
      ...
      //mudei a declaração de exampleB para cima
      expect(exampleA()).toEqual(1);
    });
    ...
    it('can create closures with several free variables', () => {
      ...
      expect(external()).toBe(undefined); // porque external() não retorna nada
    });
    ...
    it("can invoke functions indirectly using the special 'call'", () => {
      ...
      expect(f.call(f,1,1)).toEqual(2); // essa não entendi exatamente...
    });
    ...
    it("doesn't have a private scope inside blocks", () => {
      ...
      // expect(i).toEqual(undefined); // não é undefined -- na realidade, a referência não existe fora do for
      expect(j).toEqual(10);
    });
  });
  ...
  describe('has multiple ways to define and create objects', () => {
    ...
    it('has a pattern called, the Module Pattern', () => {
      ...
      expect(typeof(obj.points)).toEqual('undefined'); // o objeto não tem uma propriedade 'points'
    });

    it('may create objects also with the module pattern', () => {
      function createObject(initialScore, color) {
        return {
          incrementScoreIn: num => {
            return initialScore += num;
            },
          color: color,
          points: () => initialScore // essa tá errada, mas não sei exatamente como arrumar. hm. o teste passou
        }
      }
    ...
    it('can have have methods in the prototype', () => {
      ...
      const obj = new Obj();
      expect(obj.theName()).toEqual(undefined); // name !== this.name. é isso, certo?
      expect(obj.theName()).toBe(new Obj().theName()); //adicionei () após theName, pois são métodos
    });
  ...
  describe('commons patterns with functions and behaviors', () => {
    ...
    it("hoists variables the way you probably don't expect", () => {
      ...
      /*
      hmmm. entendi! functions no final fica:

      functions = [() => return 0, () => return 1, () => return 2, () => return 3];
  ...
  context('has ways to simulate classes', () => {
    ...
    context('and the THIS keyword', () => {
      ...
      it('works different on detached functions', () => {
        ...
        expect(window.kilos).toEqual(11); // essa não entendi muito bem!
        expect(cat.kilos).toEqual(1); // essa foi ok.
      });
      ...
      it('can be bound explicitly with CALL and APPLY', () => {
        ...
        expect(cat.kilos).toEqual(2); //adiciona 1kg ao cat de cima?
      });
      ...
      it('can be bound in modern browsers with BIND', () => {
        ...
        expect(cat.kilos).toEqual(2); // ?? por que 2?
      });
      ...
      it('works different when function is attached to other object', () => {
        ...
        otherCat.feed();
        expect(otherCat.kilos).toEqual(11); // ok
        expect(cat.kilos).toEqual(1);       // que cat é esse?
      });
      ...
      it('can be handled using the SELF trick', () => {
        ...
        expect(lion.energy).toEqual(185); // ok!
      });
      ...
      it('interprets the THIS when the function is executed', () => {
        ...
        expect(lion.energy).toEqual(4000); // inicialmente achei que sera 3985, mas por fim entendi.
      });
    });
  });
});
