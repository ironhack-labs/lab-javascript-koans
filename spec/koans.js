...
describe('the JavaScript language', () => {
  describe('has different types and operators', () => {
    ...
    it('considers nulls to be falsy', () => {
      ...
      expect(result == false).toBeTruthy();// Don't understand
      expect(null === false).toBeFalsy();// Don't understand
      expect(null == false).toBeTruthy;// Don't understand
    });
    ...
    it("can't compare arrays", () => {
      ...
      expect(a == b).toBeFalsy();  // Don't understand
      expect(a === b).toBeFalsy(); // Don't understand
    });
...
// Basically, first-class citizenship simply means “being able to do what everyone else can do.” In JavaScript, functions are objects (hence the designation of first-class object). They inherit from the Object prototype and they can be assigned key: value pairs.
  describe('considers functions as first class citizens', () => {
    ...
    it('matters, the declaration order when they are anonymous', () => {
      ...
      expect(exampleA()).toEqual("ReferenceError: Cannot access uninitialized variable. in http://127.0.0.1:5501/spec/koans.js (line 195)");
    ...
    it('defines a pure function when there are no free variables', () => {
      ...
      expect(external()).toBe(9); //Not clear for me
    });
    ...
    it("doesn't have a private scope inside blocks", () => {
      ...
      expect(i).toEqual("ReferenceError: i is not defined")
      expect(j).toEqual(10);
    });
  });
