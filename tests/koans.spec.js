describe('the JavaScript language', () => {
  describe('has different types and operators', () => {
    ...
    it('considers nulls to be falsy', () => {
      ...
      expect(null == false).toBeFalsy();// Truthy or Falsy // Dan error ambas
    });
  ...
  describe('considers functions as first class citizens', () => {
    ...
    it("doesn't care about the declaration order when they are named", () => {
      ...
      expect(exampleA()).toEqual(1); // no importa el orden mientras este declarado
    ...
    it('may return arrays that contains closures and so on', () => {
      function example() {
        ...
        return [function  (a) { 
          ...
          return ["tiki tiki ", a + 9] 
        
        } ]
      }

      expect(example()[0](1)[1]).toEqual(10); // el valor del argumento es "a" si paso a (1) = 1+9 // 
      expect(example()[0](2)[1]).toEqual(11); // el valor del argumento es "a" si paso a (2) = 2+9 // 
      expect(example()[0](3)[1]).toEqual(12); // el valor del argumento es "a" si paso a (3) = 3+9 // 
    });
